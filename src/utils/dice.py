...
def roll_wrapper(
    ...
    if not isinstance(n_repeats, int) or n_repeats < 1:
        return {
            "error": True,
            "message": templates.ROLL_WRAPPER_ROLL_WITH_REPEAT_INVALID_DIGIT.format(
                author_id=author_id
            ),
        }
    # If there are too many repetitions
        ...
        return {
            "error": True,
            "message": templates.ROLL_WRAPPER_TOO_MANY_DICE.format(
    ...
    if status == "too high":
        return {
            "error": True,
            "message": templates.ROLL_WRAPPER_DICE_TOO_HIGH.format(
                author_id=author_id,
